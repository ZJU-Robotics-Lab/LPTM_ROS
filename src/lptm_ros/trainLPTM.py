...
writer = SummaryWriter(log_dir="./checkpoints/log/train/")
...
def train_model(model_template, model_source, model_corr2softmax, model_trans_template, model_trans_source, model_trans_corr2softmax,\
    ...
    for epoch in range(start_epoch , start_epoch + num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train':
                for template, source, groundTruth_number, scale_gt, gt_trans in LPTMdataloader(batch_size)[phase]:
                    iters = iters + 1
        ...
        # forward
                    ...
                    loss_y, loss_x, total_loss, loss_l1_x,loss_l1_y,loss_mse_x, loss_mse_y, template_visual_trans, source_visual_trans \
                            = train_translation(template, source, groundTruth_number.clone(), scale_gt.clone(), gt_trans, \
                                                    model_trans_template, model_trans_source, model_trans_corr2softmax, phase, device)
        ...
        # backward + optimize only if in training phase:
                    if phase == 'train':
                        ...
                        # writer.add_scalar('angle_loss', loss_a.detach().cpu().numpy(), iters)
                        # writer.add_scalar('x_loss', loss_x.detach().cpu().numpy(), iters)
                        # writer.add_scalar('y_loss', loss_y.detach().cpu().numpy(), iters)
...
optimizer_ft_temp = optim.Adam(filter(lambda p: p.requires_grad, model_template.parameters()), lr=2e-3)
optimizer_ft_src = optim.Adam(filter(lambda p: p.requires_grad, model_source.parameters()), lr=2e-3)
...
if load_pretrained:
    ...
    if load_pretrained_mode == 'transformation':
        model_trans_template, model_trans_source,\
            start_epoch = load_trans_checkpoint(\
                                        ...
                                        device)
    if load_pretrained_mode == 'rotation':
...
model_template, model_source = train_model(model_template, model_source, model_corr2softmax, model_trans_template, model_trans_source, model_trans_corr2softmax,\
                                            ...
                                            save_path, start_epoch, num_epochs=70)
