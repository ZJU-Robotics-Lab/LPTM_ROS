...
def LPTMdataloader(batch_size):
    # use the same transformations for train/val in this example
    path = "./data"
    datacsv = pd.read_csv(path + "/data_train_qsdjt_lidar_sat/ground_truth_qsdjt_lidar.csv")
    train_upper = 14000
    val_num = 3000
    ...
    template_list = datacsv["name"].values  
    template_list = [i+".jpg" for i in template_list]
    template_list = [os.path.join(path + "/data_train_qsdjt_lidar_sat/ground/",i) for i in template_list ]
    template_train_list = template_list[:train_upper]
    template_val_list = template_list[train_upper:val_upper]

    source_list = datacsv["name"].values
    source_list = [i+".jpg" for i in source_list]
    source_list = [os.path.join(path + "/data_train_qsdjt_lidar_sat/aerial/",i) for i in source_list ]
